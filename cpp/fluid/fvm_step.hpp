...
class FVM_Step{
    ...
    vec4 *U, *U_temporary, *R, *U_left, *U_right, *U_down, *U_up, *F, *G;
    double *P, *sprad;

public:
    FVM_Step(int ni, int nj, OdeScheme ode_scheme, FluxScheme flux_scheme, BoundaryConditions boundary_conditions);
    void eval_RHS(vec4* U);
    void MUSCL_exstrapolate(vec4* U);
    void conserved2primitive(vec4* U);
    inline vec4 primitive2conserved(const vec4& V);
    inline vec4 minmod(const vec4& a, const vec4& b) const;
    void calc_P(vec4* U);
    inline double calc_P(vec4& U) const;
    void calc_F(vec4* U_hor);
    void calc_G(vec4* U_hor);
