...
namespace fluid {
    ...
    vec4 FVM_Solver::G_f_HLLC(const vec4 &U_D, const vec4 &U_U) {
        //The theory behind this implementation is found in Toro's book about Riemann solvers
        //This time it's adjusted to work vertically instead of horizontally
        double rho_D = U_D.u1;
        double rho_U = U_U.u1;
        double p_D = FVM_Solver::calc_P(U_D);
        double p_U = calc_P(U_U);
        double u_D = U_D.u2 / rho_D;
        double u_U = U_U.u2 / rho_U;
        double v_D = U_D.u3 / U_D.u1;
        double v_U = U_U.u3 / U_U.u1;
        double c_D = sqrt(Gamma * p_D / rho_D);
        double c_U = sqrt(Gamma * p_U / rho_U);

        //Step 1: Estimate wave speeds. A pressure based method estimate is chosen

        //Estimate for the star region (middle wave region) pressure
        double p_pvrs = 0.5 * (p_D + p_U) + (v_D - v_U) * (rho_D+ rho_U) * (c_D + c_U) / 8; //(10.67) Toro
        double p_star = std::max(0.0, p_pvrs); //Excluding unphysical pressure
        double q_L = (p_star <= p_D) ? 1 : sqrt(1 + (Gamma + 1) / (2 * Gamma) * (p_star / p_D - 1)); //(10.69) Toro
        double q_R = (p_star <= p_U) ? 1 : sqrt(1 + (Gamma + 1) / (2 * Gamma) * (p_star / p_U - 1)); //(10.69) Toro
