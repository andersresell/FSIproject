...
SolidBody::SolidBody(fluid::FVM_Solver &fvm, std::vector<Point> &&boundary_in, SolidBodyType type)
    : fvm{fvm}, n_bound{(int)boundary_in.size()}, ni{fvm.ni}, nj{fvm.nj}, dx{fvm.L_x / ni}, dy{fvm.L_y / nj},
      type{type}, first_timestep{true} {
    boundary = new Point[n_bound];
    F_boundary = new Point[n_bound];
    for (int i{0}; i < (int)boundary_in.size(); i++) {
        boundary[i] = boundary_in[i];
    }
    Cell::nj = nj;

    A << 1, 0, 0, 0, 1, dx, 0, 0, 1, dx, dy, dx * dy, 1, 0, dy, 0;
        ...
        for (const auto &e : cell2intercept) {
            for (int ii{-1}; ii <= 1; ii++) {
                for (int jj{-1}; jj <= 1; jj++) {
                    ...
                    if (fvm.cell_status[IX(i + ii, j + jj)] == fluid::CellStatus::Solid ||
                        !cell_within_grid(i + ii, j + jj))
                        continue;
...
void SolidBody::find_intercepts(std::map<Cell, GP_data> &intercept_map) {
    ...
    for (auto &e : intercept_map) {
        ...
        for (int j{0}; j < n_bound; j++) {
            ...
            segments[j].segment_data_vec.reserve(n_GPs_estimate); //Will remove all of this if it doesn't give any
            performance gain*/
...
void SolidBody::interpolate_fresh_points(fluid::vec4 *U_in) {
    ...
    for (auto &e : cell2intercept) {
        if (!point_inside(ind2point(e.first.i, e.first.j))) {
            cell2intercept_FP[e.first]; // Detecting the fresh points, by checking which old GP's are now outside the
                                        // boundary
        }
    }
...
void SolidBody::interpolate_cell(Cell point, const std::map<Cell, GP_data> &intercept_map, fluid::vec4 *U_in,
                                 bool fresh_point) {
    using namespace std;
        ...
        for (int ii{0}; ii < 2; ii++) {
            ...
            if (cs[ind] == fluid::CellStatus::Fluid || !cell_within_grid(i_bl + ii, j_bl + jj)) {
                ...
                if (type == SolidBodyType::Dynamic) {
                    ...
                    p_derivative_BI_adj[ind] = p_derivative_BI; // Just using the derivative already found for
                                                                // simplicity
                }
...
bool SolidBody::point_inside(Point p) const {
    ...
    for (unsigned int i{0}; i < (unsigned int)n_bound; i++) {
