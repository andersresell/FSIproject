...
class SolidBody
{
  protected:
    ...
    static constexpr double INF = 1e6; // A large number used as spatial infinity
  ...
  public:
    ...
    virtual Point get_CM_velocity() const
    {
        return {0, 0};
    } // wrote placeholder implementation to avoid vtable error
    ...
    double segment_length(int p) const
    {
        return (boundary[(p + 1) % n_bound] - boundary[p]).norm();
    }
    ...
    double interpolate_dirichlet(const Eigen::Vector4d &alpha_dir, std::array<double, 4> phi,
                                 std::array<fluid::CellStatus, 4> cs, std::array<double, 4> phi_BI_adj = {0, 0, 0, 0});
    ...
    virtual void boundary_vel_and_fluid_boundary_acc(Point BI, Point n, Point u_fluid, Point &u_wall,
                                                     double &a_normal) const {};

    virtual double max_boundary_speed() const
    {
        return 0;
    }
    ...
    bool cell_within_grid(int i, int j)
    {
        return i >= 2 && i < ni + 2 && j >= 2 && j < nj + 2;
    }
  ...
  private:
    ...
    Point ind2point(int i, int j) const
    {
        return {(i - 1.5) * dx, (j - 1.5) * dy};
    }

    Cell point2ind(double x, double y) const
    { // rounding a point down to nerest cell index
