...
class DynamicRigid : public SolidBody
{
  ...
  public:
    ...
    Point get_CM_velocity() const final
    {
        return {y[2], y[3]};
    }
  ...
  private:
    ...
    void boundary_vel_and_fluid_boundary_acc(Point BI, Point n, Point u_fluid, Point &u_wall,
                                             double &a_normal) const final;

  public:
    virtual ~DynamicRigid();
};

inline void DynamicRigid::boundary_vel_and_fluid_boundary_acc(Point BI, Point n, Point u_fluid, Point &u_wall,
                                                              double &a_normal) const
{
    ...
    Point a_CM = {(y[2] - vel_CM_prev.x) / dt_prev,
                  (y[3] - vel_CM_prev.y) / dt_prev}; // A backward difference to approximate acceleration
    ...
    // cout << "u "<<y[2]<<", u_old" << vel_CM_prev.x<<", a_CM "<<a_CM<<", alpha "<<alpha<<",omega "<<omega<< ", a_wall"
    // <<a_wall << endl; a_wall = {0, 0};
}
