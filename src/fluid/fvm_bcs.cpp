...
ExternalBCs::ExternalBCs(int ni, int nj, double dx, double dy, BC_Type west, BC_Type east, BC_Type south, BC_Type north,
                         double M_inf, double p_inf, double rho_inf, string history_output_west_folder)
    : ni{ni}, nj{nj}, dx{dx}, dy{dy}, west{west}, east{east}, south{south}, north{north} {
    if (east == BC_Type::SupersonicInflow || north == BC_Type::SupersonicInflow || south == BC_Type::SupersonicInflow) {
        throw runtime_error("Supersonic inflow bc type is only supported for western face");
    }
    ...
    if (west == BC_Type::TimeHistory) {
        ...
        load_history_output_west(history_output_west_folder);
    }
}
...
void ExternalBCs::set_BCs(vec4 *U_in, double t) {
    ...
    case BC_Type::TimeHistory: {
        ...
        auto it =
            std::find_if(time_history_west.begin(), time_history_west.end(), [t](TimeHistory x) { return x.t >= t; });
        vec4 U0, U1;
    ...
    case BC_Type::BlastLoad: {
        for (int j{0}; j < nj + 4; j++) {
            for (int i = 0; i < 2; i++) {
                Vector2 coord = get_coord(i, j);
                U_in[IX(i, j)] = blast_bc.calc_bc_val_conservative(coord, t);
            }
        }
        break;
    }
...
vec4 ExternalBCs::set_vertical_invicid_wall(const vec4 &U_in) {
    ...
    return {U_in.u1, -U_in.u2, U_in.u3, U_in.u4};
}
