...
FVM_Solver::FVM_Solver(int ni, int nj, double L_x, double L_y, double CFL, OdeScheme ode_scheme, FluxScheme flux_scheme,
    ...
    : ni{ni}, nj{nj}, L_x{L_x}, L_y{L_y}, dx{L_x / ni}, dy{L_y / nj}, CFL{CFL}, ode_scheme{ode_scheme},
      flux_scheme{flux_scheme}, limiter{limiter}, external_bcs{external_bcs}, output_folder{std::move(output_folder)} {
...
void FVM_Solver::write_fvm_output(const std::string &output_folder, int n, double t) {
    ...
    const string file = output_folder + "fvm_output_t" + std::to_string(n) + ".csv";
    std::ofstream ost{file};
    if (!ost) {
        throw runtime_error("Couldn't open fvm csv output file: " + file);
    }
    ost << "#t," << t << '\n' << "#rho,u,v,p\n";
    int ind;
    for (int i{2}; i < ni + 2; i++) {
        for (int j{2}; j < nj + 2; j++) {
            ...
            if (cell_status[ind] == CellStatus::Solid) {
                ...
                ost << V[ind].u1 << "," << V[ind].u2 << "," << V[ind].u3 << "," << V[ind].u4 << "\n";
            }
        }
    }
}
void FVM_Solver::write_fvm_header(const std::string &output_folder, int write_stride, int n_last, double t_end) const {
    ...
    const string file = output_folder + "fvm_header.csv";
    std::ofstream ost{file};
    if (!ost) {
        throw runtime_error("Couldn't open fvm csv header file: " + file);
    }
    ...
    for (int i{0}; i < ni + 4; i++) {
        for (int j{0}; j < nj + 4; j++) {
            if (i == 0 || i == 1 || i == ni + 2 || i == ni + 3 || j == 0 || j == 1 || j == nj + 2 || j == nj + 3) {
                cell_status[IX(i, j)] =
                    CellStatus::Ghost; // setting all values to fluid, including the ones at the exterior
            } else {
    ...
    for (int i = 2; i < ni + 2; i++) {
        for (int j{2}; j < nj + 2; j++) {
            if (cell_status[IX(i, j)] == CellStatus::Fluid) {
                ...
                Res[IXR(i, j)] =
                    -1 / dx * (F_f[IXF(i, j)] - F_f[IXF(i - 1, j)]) - 1 / dy * (G_f[IXG(i, j)] - G_f[IXG(i, j - 1)]);
            }
        }
    }
}
...
void FVM_Solver::MUSCL_extrapolate(vec4 *U_in) {
    ...
    const Limiter limiter_ = this->limiter;
        ...
        for (int j = 2; j < nj + 2; j++) {
            if (cell_status[IX(i, j)] != CellStatus::Solid) {
                ...
                case Limiter::Minmod:
                    ...
                    break;
                case Limiter::MC:
                    Delta = minmod(0.5 * (V[IX(i + 1, j)] - V[IX(i - 1, j)]),
                                   minmod(2 * (V[IX(i, j)] - V[IX(i - 1, j)]), 2 * (V[IX(i + 1, j)] - V[IX(i, j)])));
                    break;
                }
        ...
        for (int j = 1; j < nj + 3; j++) {
            if (cell_status[IX(i, j)] != CellStatus::Solid) {
                ...
                case Limiter::Minmod:
                    ...
                    break;
                case Limiter::MC:
                    Delta = minmod(0.5 * (V[IX(i, j + 1)] - V[IX(i, j - 1)]),
                                   minmod(2 * (V[IX(i, j)] - V[IX(i, j - 1)]), 2 * (V[IX(i, j + 1)] - V[IX(i, j)])));
                    break;
                }
