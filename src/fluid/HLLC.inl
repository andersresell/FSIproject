...
inline vec4 FVM_Solver::F_f_HLLC(const vec4 &U_L, const vec4 &U_R) {
    ...
    //(10.71) Toro
    if (S_star >= 0) {
        vec4 F_L{rho_L * u_L, rho_L * u_L * u_L + p_L, rho_L * u_L * v_L, u_L * (U_L.u4 + p_L)};
        if (S_L >= 0) {
            return F_L;
        } else { // S_L < 0
            //(10.73)
            vec4 U_star_L = {1, S_star, v_L, U_L.u4 / rho_L + (S_star - u_L) * (S_star + p_L / (rho_L * (S_L - u_L)))};
            U_star_L *= rho_L * (S_L - u_L) / (S_L - S_star);
            return F_L + S_L * (U_star_L - U_L); //(10.72)
        }
    } else { // S_star < 0
        vec4 F_R{rho_R * u_R, rho_R * u_R * u_R + p_R, rho_R * u_R * v_R, u_R * (U_R.u4 + p_R)};
        if (S_R > 0) {
            //(10.73)
            vec4 U_star_R = {1, S_star, v_R, U_R.u4 / rho_R + (S_star - u_R) * (S_star + p_R / (rho_R * (S_R - u_R)))};
...
inline vec4 FVM_Solver::G_f_HLLC(const vec4 &U_D, const vec4 &U_U) {
