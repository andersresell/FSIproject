...
void simulate(const std::string &input_file) {
...

    {
        ...
        input_parser.create_solvers(fvm, fsi, bcs);
...
//     fluid::ExternalBCs bcs{ni,    nj,    west_bc, east_bc, south_bc, north_bc, time_history_west_folder,
//                            M_inf, p_inf, rho_inf};
...
//                 M = 2 * h * l * rho;
//                 I = rho / 3 * h * l * (sqr(h) + sqr(l)); // Calculated by solving a double integral, might be wrong
//                 CM = {x_center, y_center};
//             }
