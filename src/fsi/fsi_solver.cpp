...
int FSI_Solver::solve() {
    ...
    while (true) {
        ...
        if (stopping_crit.first == StoppingCrit::Time) {
            if (t >= stopping_crit.second)
                breaker = true;
        } else if (stopping_crit.first == StoppingCrit::Timesteps) {
            if (n >= (int)stopping_crit.second)
                breaker = true;
        ...
        if (breaker) {
            if (n % fvm_write_stride != 0)
                fvm.write_fvm_output(output_folder, n, t); // making sure that last step is read
...
double FSI_Solver::calc_density_L2_norm() {
    ...
    for (int i{2}; i < fvm.ni + 2; i++) {
        for (int j{2}; j < nj + 4; j++) {
            if (fvm.cell_status[IX(i, j)] == fluid::CellStatus::Fluid)
                res_norm += sqr(fvm.U[IX(i, j)].u1 - rho_old[IX(i, j)]);
        }
    }
...
void FSI_Solver::write_history_output_west() {
    if (history_output_west_enabled) {
        ...
        for (auto &e : history_output_west) {
            ...
            ost << e.t << ' ' << U0.u1 << ' ' << U0.u2 << ' ' << U0.u3 << ' ' << U0.u4 << ' ' << U1.u1 << ' ' << U1.u2
                << ' ' << U1.u3 << ' ' << U1.u4 << '\n';
        }
    }
}
