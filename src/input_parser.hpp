...
class InputParser
{
  ...
  private:
    ...
    void add_parsed_option(string root_name, string option_name);
    void report_invalid_options() const;
    void check_that_root_name_is_valid(string root_name) const;

    template <typename T> T read_required_option(string root_name, string option_name, string extra_msg = "")
    {
        add_parsed_option(root_name, option_name);
        check_that_root_name_is_valid(root_name);
    ...
    template <typename EnumType>
    EnumType read_required_enum_option(string root_name, string option_name, const std::map<string, EnumType> &enum_map)
    {
        add_parsed_option(root_name, option_name);
        check_that_root_name_is_valid(root_name);
        if (root_node[root_name][option_name])
    ...
    const vector<string> available_root_nodes = {"setup", "external_bcs", "initial_condition", "solids"};
    ...
    static inline const map<string, fluid::OdeScheme> ode_scheme_from_string{
        {"explicit_euker", fluid::OdeScheme::ExplicitEuler}, {"tvd_rk3", fluid::OdeScheme::TVD_RK3}};
